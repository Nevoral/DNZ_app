...
func SignUpTab() lx.Content {
	return html.Div(
		...
		signupForm(),
	).Id("signupTab").Class("flex flex-col justify-center items-center w-1/2 p-8 bg-white text-black shadow-lg rounded-lg bg-opacity-50 backdrop-blur-sm")
}

func signupForm() lx.Content {
	return html.Form(
		...
		html.Button("Registrovat").Disabled().Type("button").Id("RegisterBTN").
			...
			CustomAtr("hx-post", "/signup").CustomAtr("hx-target", "#signupTab").CustomAtr("hx-swap", "outerHTML"),
		html.Div(
			html.Span("Už máš vlastní účet? ").Class("text-gray-800 text-xs"),
			html.A("Přihlas se").Class("text-black underline").
				CustomAtr("hx-get", "/logintab").CustomAtr("hx-target", "#signupTab").CustomAtr("hx-swap", "outerHTML"),
		).Class("text-center"),
	).Id("signupForm").Class("flex flex-col")
}
		...
		html.Div(
			html.Div(
				html.Input().Type(inputType).Name(id).Placeholder(placeholder).Id(id).
					Class("w-full text-base focus:outline-none bg-white bg-opacity-0 rounded-full"),
				element,
			).Class("flex flex-row justify-between items-center mx-4 w-full"),
		).Id(id+"Shadow").Class("flex flex-row shadow-lg bg-white bg-opacity-75 text-neutral-800 rounded-full pb-1 w-full h-10 hover:bg-opacity-100"),
		html.Span("").Id(id+"MsgBox").Class("relative w-auto text-neutral-800 p-1 px-2 rounded-xl bg-red-500 bg-opacity-0 text-xs mx-4 font-bold -top-2"),
	).Class("flex flex-col justify-center items-start mb-2 w-72")
}

func CircleButton(image lx.Content) lx.Content {
	return html.Button(image).Class("bg-white bg-opacity-75 hover:bg-opacity-100 rounded-full p-4 shadow-lg")
}
